...
const readJobFile = async (year, name) => {
    const jobPath = path.join(process.env['BMC_METADATA_DIR'], year, name, 'index.json')
...
const streamJobImage = ({year, jobname, filename, size, scale}) => {
    ...
    const jobPath = path.join(process.env['BMC_THUMBNAILS_DIR'], year, jobname, `${filename}_${size}${scaleSuffix}.jpg`)
...
router.get('/job/:year/:name', async (ctx, next) => {
    ...
    try {
        ctx.type = 'json'
        ctx.body = fs.createReadStream(path.join(process.env['BMC_METADATA_DIR'], ctx.params.year, ctx.params.name, 'index.json'))
    } catch (err) {
        console.error(err)
...
router.get('/job/:year/:name/featured/:size/:scale?', async (ctx, next) => {
    ...
    try {
        ...
        ctx.type = 'jpg'
        ctx.body = streamJobImage({year, jobname, filename: featuredPhoto.filename, size, scale})
    } catch (err) {
        console.error(err)
        if (err.statusCode === 'ENOENT') {
            ctx.throw(404)
...
router.get('/job/:year/:name/photo/:filename/:size/:scale?', async (ctx, next) => {
    await next()
    try {
