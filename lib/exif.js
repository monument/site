...
const tags = require('./exif-tags.json')
const fs = require('fs')
...
function checkAPPn(buffer) {
  try {
    ...
    const isInRange = APPMarkerTag >= APPMarkerBegin &&
      APPMarkerTag <= APPMarkerEnd
...
function IFDHandler(buffer, tagCollection, order, offset) {
  ...
  for (entryCount; entryCount < entriesNumber; entryCount += 1) {
    ...
    if (dataLength > 4) {
      const dataOffset = (order
        ? dataValue.readUInt32BE(0)
        : dataValue.readUInt32LE(0)) - offset
      ...
      switch (dataFormat) {
        ...
        case 5:
          ...
          for (let i = 0; i < dataValue.length; i += 8) {
            const bigTagValue = dataValue.readUInt32BE(i) /
              dataValue.readUInt32BE(i + 4)
            const littleTagValue = dataValue.readUInt32LE(i) /
              dataValue.readUInt32LE(i + 4)
        ...
        case 10: {
          ...
          const littleOrder = dataValue.readInt32LE(0) /
            dataValue.readInt32LE(4)
 ...
 */
exports.parseSync = function sync(file) {
  if (!file) {
 ...
 * }
 */
module.exports.parse = function async(file, callback) {
  data = undefined
  new Promise(
    (resolve, reject) => {
      if (!file) {
